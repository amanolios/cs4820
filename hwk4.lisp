...
; Q1a. We are using the definition on page 139
...
(property (x y acc :tl)
    (=> (^ (consp x) (endp y))
     (< (m-bad-app y x acc) (m-bad-app x y acc))))
     
(property (x y acc :tl)
    (=> (^ (endp x) (consp y))
     (< (m-bad-app x (rest y) (cons (first y) acc)) (m-bad-app x y acc))))

(property (x y acc :tl a :all)
    (=> (^ (consp x) (consp y))
     (< (m-bad-app x nil a) (m-bad-app x y acc))))

(property (x y acc :tl)
    (=> (^ (consp x) (consp y))
     (< (m-bad-app acc nil y) (m-bad-app x y acc))))
...
(property bad-app-app-gen (x y acc :tl)
  (== (bad-app x y acc)
      (if (^ (consp x) (consp y))
	  (append (rev x) (rev acc) y)
	  (append (rev x) (rev y) acc))))
...
; Q2a. We are using the definition on page 139

; Note: fill in the ...'s above, as you can use the generalized
; measure functions, as mentioned in Section 5.5.

; Q2b. Fill in the definition
(definec m-ack (n m :all) :all
  (if (posp n)
      (make-ord 1 n (nfix m))
    (nfix m)))
...
; Q2c
(property (n m :nat)
    :hints (("Goal" :in-theory (enable m-ack)))
    (=> (^ (!= 0 n) (== 0 m))
	(o< (m-ack (1- n) 1) (m-ack n m))))
  
(property (n :nat a :all)
    :hints (("Goal" :in-theory (enable m-ack)))
    (=> (^ (!= 0 n) (!= 0 a))
	(o< (m-ack (1- n) a) (m-ack n a))))

(property (n m :nat)
    :hints (("Goal" :in-theory (enable m-ack)))
    (=> (^ (!= 0 n) (!= 0 m))
	(o< (m-ack n (1- m)) (m-ack n m))))
...
; Q3a. We are using the definition on page 139
...
(definec m-if-flat (x :if-expr) :pos
	 (match x
	   (:if-atom 1)
	   (('if a b c) (* (m-if-flat a) (+ 1 (m-if-flat b) (m-if-flat c))))))
...
; Q3c
(property (x :if-expr)
    (=> (^ (! (if-atomp x)) (if-atomp (second x)))
     (< (m-if-flat (third x)) (m-if-flat x))))
     
(property (x :if-expr)
    (=> (^ (! (if-atomp x)) (if-atomp (second x)))
     (< (m-if-flat (fourth x)) (m-if-flat x))))

(property (b c d e f :if-expr)
    (< (m-if-flat (list 'if d (list 'if e b c) (list 'if f b c)))
     (m-if-flat (list 'if (list 'if d e f) b c))))
...
(property helper-if-flat-equal (e3 e4 :all a :if-assign)
    (=> (^ (! (consp e3))
	   (if-exprp (list* 'if e3 e4))
	   (consp e4)
	   (consp (cdr e4))
	   (not (cddr e4))
	   (not (booleanp e3))
	   (not (varp e3)))
     (not (if-eval (list* 'if e3 e4) a))))
  
(property if-flat-equal-if (a :if-assign e :if-expr)
  (== (if-eval (if-flat e) a) (if-eval e a)))
...
(definec agrees-with (b a :if-assign) :bool
  (match b
    (nil t)
    (((v . val) . r)
     (and (equal (lookup-var v a) val)
          (agrees-with r a)))))

(property agrees-with-acons-t (x :var a b :if-assign)
  (=> (^ (agrees-with b a) (lookup-var x a))
      (agrees-with (acons x t b) a)))

(property agrees-with-acons-nil (x :var a b :if-assign)
  (=> (^ (agrees-with b a) (! (lookup-var x a)))
      (agrees-with (acons x nil b) a)))

(property agrees-with-lookup-atom (x :if-atom a b :if-assign)
  (=> (^ (agrees-with b a) (assignedp x b))
      (equal (lookup-atom x b) (lookup-atom x a))))

(property agrees-with-lookup-var (e :var a b :if-assign)
  (=> (^ (agrees-with b a) (lookup-var e b))
      (lookup-var e a)))

(property validp-sound (e :norm-if-expr a b :if-assign)
  (=> (^ (validp e b) (agrees-with b a))
      (if-eval e a)))

(property check-validp-is-sound (a :if-assign e :if-expr)
  :hints (("Goal" :use ((:instance validp-sound
                          (e (if-flat e)) (b nil)))))
  (=> (check-validp e)
      (if-eval e a)))
...
(definec find-nil-validity (e :norm-if-expr a :if-assign) :if-assign
	 (match e
	     (:if-atom a)
	   (('if x y z)
	    (if (assignedp x a)
		(if (lookup-atom x a)
		    (find-nil-validity y a)
		    (find-nil-validity z a))
		(if (! (validp y (acons x t a)))
		    (find-nil-validity y (acons x t a))
		    (find-nil-validity z (acons x nil a)))))))

(property (e :if-expr)
    (=> (! (check-validp e))
     (! (if-eval e (find-nil-validity (if-flat e) nil)))))
...
(property orderedp-insert (x :rational l :tl)
  :hyps (orderedp l)
  (orderedp (insert x l)))
...
#|
(property qsort-orderedp (x :tl)
   (orderedp (qsort x)))

(property qsort-permp (x :tl)
(permp (qsort x) x))
|#
