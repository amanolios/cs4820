...
; SAT - ((P "Aba") (Y "ab") (X "Ab") (Z "CB") (Q "bCB"))
...
(z3-assert (x (:seq :bool) y :int)
	   (and (<= 0 y)
		(<= y 32)
		(= (seq.len x) y)
		(ite (and (>= (seq.len x) 1) (seq.nth x 0))
		     (= (mod (seq.len x) 2) 0)
		     (= (mod (seq.len x) 2) 1))))
...
; SAT - ((Y 1) (X (NIL))
; (|seq.nth_u|
;  '(:FN ((#<Z3-SORT (Seq Bool)> #<Z3-SORT Int>) #<Z3-SORT Bool>) ((:DEFAULT)))))
...
(solver-reset)
(solver-push)
(z3-assert (Alice Bob Clara :bool alice-num :int)
	   (and (<= 0 alice-num)
		(<= alice-num 3)
		(= Alice (= alice-num (+ (ite Alice 1 0) (ite Bob 1 0) (ite Clara 1 0))))
		(= Bob (= alice-num 1))
		(= Clara (not Bob))))
(check-sat)
(get-model-as-assignment)
...
; SAT - ((CLARA T) (ALICE-NUM 0) (BOB NIL) (ALICE NIL))
; Alice - knave + said 0, Bob - knave, Clara - knight 
...
(solver-reset)
(solver-push)
(register-enum-sort :job (driver guard fireman))
(z3-assert (mrfireman mrguard mrdriver :job)
           (and (distinct mrfireman mrguard mrdriver)
                (= (+ (ite (!= mrdriver (enumval :job guard)) 1 0)
                      (ite (!= mrfireman (enumval :job driver)) 1 0)
                      (ite (= mrdriver (enumval :job driver)) 1 0)
                      (ite (!= mrfireman (enumval :job guard)) 1 0))
                   1)))
(check-sat)
(get-model-as-assignment)
(solver-pop)
...
; SAT - Mr.Fireman is Driver, Mr.Guard is Fireman, Mr.Driver is Guard 
...
(defun 3x3-magic-square-diag-constraints (sum-var)
...
(defun 3x3-magic-square-all-distinct ()
  (let ((all-vars (loop for row below 3 append
			(loop for col below 3 collect (get-3x3-magic-square-var row col)))))
    `(distinct ,@all-vars)))
...
(z3-assert-fn (3x3-magic-square-var-specs 'S)
              (3x3-magic-square-vars-between-1-and-9))
(z3-assert-fn (3x3-magic-square-var-specs 'S)
              (3x3-magic-square-diag-constraints 'S))
(z3-assert-fn (3x3-magic-square-var-specs 'S)
...
(defun sudoku-cell-constraints ()
  `(and ,@(loop for row below 9 append
                (loop for col below 9
                      collect (exactly-one
...
(defun sudoku-col-constraints ()
  `(and ,@(loop for col below 9 append
                (loop for val from 1 to 9
                      collect (exactly-one
                               (loop for row below 9
                                     collect (sudoku-cell-var row col val)))))))

 ; Each 3x3 box has each value exactly once
(defun sudoku-box-constraints ()
  `(and ,@(loop for box-row below 3 append
                (loop for box-col below 3 append
                      (loop for val from 1 to 9
                            collect (exactly-one
                                     (loop for r from (* box-row 3)
                                           below (+ (* box-row 3) 3) append
                                           (loop for c from (* box-col 3)
                                                 below (+ (* box-col 3) 3)
                                                 collect (sudoku-cell-var r c val)))))))))

; Constraints from the starting board
(defun sudoku-starting-constraints (board)
  `(and ,@(loop for i below 81
                for val in board
                when (numberp val)
                collect (sudoku-cell-var (floor i 9) (mod i 9) val))))

(defun solve-sudoku (board)
  (solver-reset)
  (solver-push)
  (let ((specs (sudoku-var-specs)))
    (z3-assert-fn specs (sudoku-cell-constraints))
    (z3-assert-fn specs (sudoku-row-constraints))
    (z3-assert-fn specs (sudoku-col-constraints))
    (z3-assert-fn specs (sudoku-box-constraints))
    (z3-assert-fn specs (sudoku-starting-constraints board)))
  (let ((status (check-sat)))
    (if (eq status :sat)
        (let ((assignment (get-model-as-assignment)))
          (solver-pop)
          assignment)
...
(defun sudoku-range-constraints-alt ()
  `(and ,@(loop for row below 9 append
                (loop for col below 9 append
                      `((<= 1 ,(sudoku-cell-var-alt row col))
                        (<= ,(sudoku-cell-var-alt row col) 9))))))

(defun sudoku-row-constraints-alt ()
  `(and ,@(loop for row below 9
                collect `(distinct ,@(loop for col below 9
                                           collect (sudoku-cell-var-alt row col))))))

(defun sudoku-col-constraints-alt ()
  `(and ,@(loop for col below 9
                collect `(distinct ,@(loop for row below 9
                                           collect (sudoku-cell-var-alt row col))))))

(defun sudoku-box-constraints-alt ()
  `(and ,@(loop for box-row below 3 append
                (loop for box-col below 3
                      collect `(distinct ,@(loop for r from (* box-row 3)
                                                below (+ (* box-row 3) 3) append
                                                (loop for c from (* box-col 3)
                                                      below (+ (* box-col 3) 3)
                                                      collect (sudoku-cell-var-alt r c))))))))

(defun sudoku-starting-constraints-alt (board)
  `(and ,@(loop for i below 81
                for val in board
                when (numberp val)
                collect `(= ,(sudoku-cell-var-alt (floor i 9) (mod i 9)) ,val))))

(defun get-square-value-alt (soln row col)
  (let ((entry (assoc-equal (sudoku-cell-var-alt row col) soln)))
    (when entry (cadr entry))))

(defun pretty-print-3x3-sudoku-solution-alt (soln)
  (loop for row below 9
        do (progn (terpri)
                  (loop for col below 9
                        do (progn (format t "~A " (get-square-value-alt soln row col))
                                  (when (equal (mod col 3) 2) (format t "  "))))
                  (when (equal (mod row 3) 2) (terpri)))))
...
(defun solve-sudoku-alternate (input-grid)
  (solver-reset)
  (solver-push)
  (let ((specs (sudoku-var-specs-alt)))
    (z3-assert-fn specs (sudoku-range-constraints-alt))
    (z3-assert-fn specs (sudoku-row-constraints-alt))
    (z3-assert-fn specs (sudoku-col-constraints-alt))
    (z3-assert-fn specs (sudoku-box-constraints-alt))
    (z3-assert-fn specs (sudoku-starting-constraints-alt input-grid)))
  (let ((status (check-sat)))
    (if (eq status :sat)
        (let ((assignment (get-model-as-assignment)))
          (solver-pop)
...
(pretty-print-3x3-sudoku-solution-alt (time (solve-sudoku-alternate *sudoku-example-board*)))
